...
class EvolutionFinder:
    def __init__(self, surrogate, model, **kwargs):
    ...
    def random_valid_sample(self, constraint):
        while True:
            sample = self.arch_manager.random_sample_arch(self.model)
            efficiency = self.surrogate.predict_acc([sample])[1].cpu().item()
    ...
    def mutate_sample(self, sample, constraint):
        while True:
            ...
            self.arch_manager.mutate_arch(new_sample, self.arch_mutate_prob, self.model)

            efficiency = self.surrogate.predict_acc([new_sample])[1].cpu().item()
            if efficiency <= constraint:
    ...
    def run_evolution_search(self, constraint, verbose=False, **kwargs):
        ...
        accs = self.surrogate.predict_acc(child_pool)[:, 0]
