...
from test_utils import INP as inp, DEVICES, MODELS, METHODS, SIGMA, get_net_transformed, nn2memse
...
def test_memristor_manips(method, device, net, sigma):
    ...
    if DEBUG:
        mses, means, varis, covs = memse.mse_forward(inp, compute_power=False, reps=1e4)
        print(means)
        print(varis)
        print(covs)
        print('----------')
